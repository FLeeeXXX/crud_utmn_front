...
const Main = ({title, newsList, NewsClickHandler, currentNews, NewsDeleteHandler, NewsCreateHandler}) =>{
    ...
    return(
        <main>
            ...
            <section id="news-list">
                {
                    viewModal && <NewsInfo closeNewsModal={closeNewsModal} NewsCreateHandler={NewsCreateHandler}/>
                }
                <h2>{title}</h2>
                <button onClick={() => addNewsHandler()}>Добавить новость</button>
                {newsList.length > 0 && newsList ? newsList.map((news) => (
